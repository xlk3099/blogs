...
只支持单进程访问。意味着用方法2拉数据的时候，eth节点得挺，可是有部分数据是必须通过rpc获取，不能停啊
...
1. 所有数据都是从eth eth拿，速度非常慢， 但并没有对查询进行优化 如果后续有新的需求或者说parser部分出错，重新导入绝对是个大工程。
2. 数据类型偏少，需要查询账户相关信息。
...
试了一遍，好几次都是半夜有个想法，是先到凌晨4，5点。

同时也是认识到自己的毛躁，因为改的频繁，中间出了几次regression...**论测试的重要性**
